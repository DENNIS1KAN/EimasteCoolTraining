...
function fmtDateParts(date: Date, opts: Intl.DateTimeFormatOptions): string {
  return new Intl.DateTimeFormat(LOCALE, opts).format(date).replace(/\bSept\b/g, 'Sep')
}
...
/** Short duration in whole minutes: 3900000 -> "1 h 5 min"; any positive time reads at least "1 min". */
