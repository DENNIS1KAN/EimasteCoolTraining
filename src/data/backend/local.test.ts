...
describe('LocalBackend.reset', () => {
  it('opens clean: the squad, and nothing logged for them yet', async () => {
    ...
    // the people are still there to sign in as
    expect(snap.members.length).toBe(3)
    expect(snap.members.find((m) => m.id === DEMO_IDS.stelios)?.goal).toBe('')
  })

  it('"Start empty" keeps the people but gives them the profile of a new member', async () => {
    const b = new LocalBackend()
    b.reset(true) // bring the sample squad in first, so there is something to clear
    ...
    for (const m of snap.members) {
      ...
      expect(m).toMatchObject({ goal: '', goalWeightKg: null, heightCm: null, programStart: null })
