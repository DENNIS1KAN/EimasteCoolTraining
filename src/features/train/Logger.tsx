...
import { bestE1rmByExercise, isPRSet, nextWorkout, programWeekOn, refKey, workoutOn } from '../../lib/stats'
import { Banner, Button, PageHeader, Tag, celebrate, cx, toast, useIsDesktop, useWakeLock } from '../../ui'
...
import { buildHistoryIndex, previousPerformance } from './logic/previous'
...
export function Logger({ me, program, week, day }: Props) {
  ...
  const next = useMemo(
    () => (logDone && log ? nextWorkout(program, [...others.filter((l) => l.programId === program.id), log]) : null),
    [logDone, log, others, program],
  )
  ...
  const [open, setOpen] = useState<Set<number>>(() => {
    if (isDesktop) return new Set(pday.ex.map((_, i) => i))
  ...
  const ctx = useRef({ me, program, week, day, pday, machines, priorBest, isDesktop, t })
  ctx.current = { me, program, week, day, pday, machines, priorBest, isDesktop, t }
  ...
  const actions = useMemo<ExerciseActions>(() => {
    ...
    const advance = (from: number, to: number) => {
      setOpen((prev) => {
        ...
        if (!ctx.current.isDesktop) n.delete(from)
        n.add(to)
        return n
      })
      setTimeout(() => scrollTo(to), 60)
    }
    ...
    return {
      ...
      tick: (i, j, ph) => {
        ...
        startRest({ seconds: restSeconds(ex(i).rest), next: caption(nextLog, up), route: `/train/${c.week}/${c.day}` })
        if (up.kind === 'exercise') advance(i, up.exercise)
        return result
      },
  ...
  const finish = (p: FinishInput) => {
    const cur = getState().logs[id] ?? emptyLog(me.id, program.id, week, day, me.settings.unit)
